...
help_message <- "This script is used to separately associate e-traits (gene expression levels) with the cis- and trans-components of gene expression to identify upstream regulatory factors that influence e-trait expression.
Usage: Rscript cistrans_etwas.R [options]
...
for (arg in args) {
  ...
  else if (startsWith(arg, "--gene_file=")) {
    gene_file <- gsub("--gene_file=", "", arg)
  }
...
pheno <- t(exp_m[match(genes,rownames(exp_m)),])
